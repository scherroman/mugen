...
def get_events() -> EventList:
    return EventList([Silence(6), Beat(12), Beat(18), Beat(24), Silence(30)])
...
def get_events_speed_multiplied_2x() -> EventList:
    return EventList(
        [Silence(6), Beat(12), Beat(15), Beat(18), Beat(21), Beat(24), Silence(30)]
    )
...
@pytest.mark.parametrize(
    "events, speed, offset, expected_events",
    [
...
@pytest.mark.parametrize(
    "events, offset, expected_locations",
    [
        (get_events(), 0.5, [6.5, 12.5, 18.5, 24.5, 30.5]),
...
@pytest.mark.parametrize(
    "events_a, events_b, expected_events",
    [
        (
            EventList([6, 12], end=30),
            EventList([18, 24], end=30),
            EventList([6, 12, 18, 24], end=30),
        ),
        (
            EventList([6, 12], end=18),
            EventList([18, 24], end=30),
            EventList([6, 12, 18, 24], end=30),
        ),
    ],
...
@pytest.mark.parametrize(
    "events, expected_event_group_list",
    [
...
@pytest.mark.parametrize(
    "events, slices, expected_event_groups",
    [
        ...
        (
            EventList([1, 2, 3, 4, 5]),
            [(0, 3), (3, 4)],
            EventGroupList([[1, 2, 3], [4], [5]]),
        ),
        (
            EventList([1, 2, 3, 4, 5, 6, 7, 8]),
            [(1, 3), (5, 7)],
            EventGroupList([[1], [2, 3], [4, 5], [6, 7], [8]]),
        ),
    ],
)
...
def test_event_group_list__group_by_types_resulting_groups():
    ...
    assert event_groups.selected_groups == EventGroupList(
        [EventList([Silence(6)]), EventList([Silence(30)])]
    )
    assert event_groups.unselected_groups == EventGroupList(
        [EventList([Beat(12), Beat(18), Beat(24)])]
    )
...
@pytest.mark.parametrize(
    "event_groups, speeds, offsets, expected_event_groups",
    [
        (
            EventGroupList([[1, 2, 3], [4], [5]]),
            [1 / 2],
            None,
            EventGroupList([[1, 3], [4], [5]]),
        ),
        (
            EventGroupList([[1, 2, 3], [4], [5]]),
            [1 / 2, 1 / 4, 1 / 8],
            [1],
            EventGroupList([[2], [4], [5]]),
        ),
        (
            EventGroupList([[1], [2, 3, 4, 5], [4, 5], [6, 7, 8, 9], [8]]),
            [1, 1 / 2, 2, 1 / 4, 0],
            None,
...
def test_event_group_list__speed_multiply(
    event_groups, speeds, offsets, expected_event_groups
):
...
def test_event_group_list__flatten():
    assert EventGroupList(
        [EventList([1, 2, 3], end=50), EventList([4, 5, 6], end=50)]
    ).flatten() == EventList([1, 2, 3, 4, 5, 6], end=50)
