...
def get_failing_filter_combo():
    return [get_has_text_filter(), get_has_majority_red_color_filter(), get_has_majority_green_color_filter()]


def get_failing_filter_combo_short_circuit():
    return [get_has_majority_green_color_filter(), get_has_text_filter(), get_has_majority_red_color_filter()]


def get_passing_filter_combo():
    return [get_has_text_filter(), get_has_majority_color_filter(), get_is_repeat_context_filter()]


@pytest.mark.parametrize("filters, expected_num_passed, expected_num_failed", [
    (get_failing_filter_combo(), 2, 1),
    (get_passing_filter_combo(), 3, 0)
])
def test_apply_filters__returns_proper_number_passed_failed(filters, expected_num_passed,
                                                            expected_num_failed):
...
def test_apply_filters___tests_all_filters_when_short_circuit_is_false():
    passed_filters, failed_filters = Filterable().apply_filters(get_failing_filter_combo_short_circuit(), short_circuit=False)
