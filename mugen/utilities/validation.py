...
def validate_speed_multiplier(func):
    ...
    def _validate_speed_multiplier(*args, **kwargs):
        ...
        if speed_multiplier:
            ...
            if speed_multiplier == 0 or (
                speed_multiplier.numerator != 1 and speed_multiplier.denominator != 1
            ):
                raise ValueError(
                    f"Improper speed multiplier {speed_multiplier}. Speed multipliers must be of the form x or 1/x, where x is a natural number."
                )

        if speed_multiplier_offset:
            if speed_multiplier >= 1:
                raise ValueError(
                    f"Improper speed multiplier offset {speed_multiplier_offset} for speed multiplier {speed_multiplier}. Speed multiplier offsets may only be used with slowdown speed multipliers."
                )
            elif speed_multiplier_offset > speed_multiplier.denominator - 1:
                raise ValueError(
                    f"Improper speed multiplier offset {speed_multiplier_offset} for speed multiplier {speed_multiplier}. Speed multiplier offset may not be greater than x - 1 for a slowdown of 1/x."
                )
